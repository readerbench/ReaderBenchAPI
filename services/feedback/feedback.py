...
def compute_textual_indices(text, lang):
    print(text)
...
def automatic_scoring(indices):
    url = 'services/feedback/textual_indices.csv'
    X, y = prepare_dataset(url)
    with open("services/binaries/gbr.pkl", "rb") as f:
        model = pickle.load(f)
    ...
    print("score: {0}".format(round(prediction[0], 2)))
...
def filter_dataset_PCA():
    if path.exists('services/binaries/pca-dataset.pkl'):
        print('pca-dataset exists')
        with open('services/binaries/pca-dataset.pkl', 'rb') as f:
        ...
        pickle.dump(dataset, open('services/binaries/pca-dataset.pkl', 'wb'))

    return dataset


def create_PCA(dataset):
    if path.exists('services/binaries/pca.pkl') and path.exists('services/binaries/pca-components.pkl'):
        print('pca & pca-components exist')
        with open('services/binaries/pca.pkl', 'rb') as f:
            pca = pickle.load(f)
        with open('services/binaries/pca-components.pkl', 'rb') as f:
        ...
        pickle.dump(pca, open('services/binaries/pca.pkl', 'wb'))
        principalComponents = pca.fit_transform(textual_indices_df)
        print('creating pca-components.pkl')
        pickle.dump(principalComponents, open('services/binaries/pca-components.pkl', 'wb'))
...
def automatic_feedback_granularity(doc_indices, granularity, feedback_metrics):
    ...
    for metric in feedback_metrics[granularity]:
        if metric.get('type', None) == 'aggregate':
            value = 0
            if metric['operation'] == 'sum':
                for index_id in metric['indices']:
                    value += indices.get(index_id, 0) if indices.get(index_id, 0) is not None else 0
                indices[metric['id']] = value

        if metric.get('min') and indices.get(metric['id']):
            if indices[metric['id']] <= metric['min']:
                feedback.append({
                    'name': metric['name'],
                    'description': metric['feedbackMessagesLow'][randrange(len(metric['feedbackMessagesLow']))]
                })
        if metric.get('max') and indices.get(metric['id']):
...
def automatic_feedback(doc_indices):
    url = 'services/feedback/feedback_rules.json'
    feedback_metrics = get_feedback_metrics(url)

    print("feedback_metrics: ")
...
#     print(automatic_feedback(indices))
